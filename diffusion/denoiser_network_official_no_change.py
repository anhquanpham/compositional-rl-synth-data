...
class ResidualBlock(nn.Module):
    def __init__(self, encoder_kwargs, observation_shape):
        ...
        # #self.Residual = nn.Linear(2048, 2048) #JUST ADDED
        #self.linear = nn.Linear(164*projection_factor, 164*projection_factor)  # Final transformation before residual connection
    ...
    def forward(self, x):
        ...
        #learned_residual = self.residual_transform(learned_residual)
        #learned_residual = self.residual_transform(x[:, :164])
        
        # Apply LayerNorm + Activation + Final Linear (like OLDResidualBlock)
        # print("Shape after Layer norm: ", self.ln(learned_residual).shape)
        # print("Shape after activation: ", self.activation(self.ln(learned_residual)))
        learned_residual = self.activation(self.ln(learned_residual)) #THIS IS THE ORIGINAL
        #transformed_residual = self.activation(self.ln(learned_residual))
        #print("Transformed")

        # residual_output = learned_residual
        # for _ in range(6):
        #     residual_output = self.Residual(self.activation(self.ln(residual_output)))

        # transformed_residual = self.linear(residual_output)
